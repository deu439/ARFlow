...
class FlowEstimatorDense(nn.Module):
    def __init__(self, ch_in, ch_out=2):
        ...
        self.conv_last = conv(ch_in + 448, ch_out, isReLU=False)
...
class FlowEstimatorReduce(nn.Module):
    ...
    def __init__(self, ch_in, ch_out=2):
        ...
        self.feat_dim = 32
        self.predict_flow = conv(64 + 32, ch_out, isReLU=False)
...
class PWCLiteProb(nn.Module):
    def __init__(self, cfg):
        super(PWCLiteProb, self).__init__()
        self.search_range = 4
        self.ch_out = 4
        ...
        self.num_ch_in = 32 + (self.dim_corr + self.ch_out) * (self.n_frames - 1)

        if self.reduce_dense:
            self.flow_estimators = FlowEstimatorReduce(self.num_ch_in, self.ch_out)
        else:
            self.flow_estimators = FlowEstimatorDense(self.num_ch_in, self.ch_out)

        self.context_networks = ContextNetwork(
            (self.flow_estimators.feat_dim + self.ch_out) * (self.n_frames - 1), self.ch_out)
    ...
    def forward_2_frames(self, x1_pyramid, x2_pyramid):
        ...
        log_var = torch.ones(b_size, 2, h_x1, w_x1, dtype=init_dtype,
                           device=init_device).float()
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            if l == 0:
                ...
                log_var = F.interpolate(log_var + 2*math.log(2), scale_factor=2,
                                     mode='bilinear', align_corners=True)
            ...
            # estimate flow and log variance
            x1_1by1 = self.conv_1x1[l](x1)
            x_intm, out = self.flow_estimators(torch.cat([out_corr_relu, x1_1by1, flow, log_var], dim=1))
            flow_res, log_var_res = torch.split(out, 2, dim=1)
            flow = flow + flow_res
            log_var = log_var + log_var_res

            # refine flow and log variance
            out = self.context_networks(torch.cat([x_intm, flow, log_var], dim=1))
            flow_fine, log_var_fine = torch.split(out, 2, dim=1)
            flow = flow + flow_fine
        ...
        if self.upsample:
            ...
            log_vars = [F.interpolate(log_var + 2 * math.log(4), scale_factor=4, mode='bilinear', align_corners=True) for log_var in log_vars]

        output = [torch.cat([flow, log_var], 1) for flow, log_var in zip(flows, log_vars)]
        return output[::-1]
