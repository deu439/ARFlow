...
torch::Tensor forward_substitution(torch::Tensor A, torch::Tensor B, torch::Tensor C, torch::Tensor D, torch::Tensor X) {
    // Check inputs
    CHECK_INPUT(A);
    CHECK_INPUT(B);
    CHECK_INPUT(C);
    ...
    return forward_substitution_cuda(A, B, C, D, Y);
}

torch::Tensor backward_substitution(torch::Tensor A, torch::Tensor B, torch::Tensor C, torch::Tensor D, torch::Tensor X) {
    // Check inputs
    CHECK_INPUT(A);
    CHECK_INPUT(B);
    CHECK_INPUT(C);
    ...
    // Initialize output array & run
    torch::Tensor Y = torch::clone(X);
    return backward_substitution_cuda(A, B, C, D, Y);
}
