...
def get_dataset(all_cfg):
    ...
    for cfg in cfgs:
        if cfg.type == 'Sintel_Flow':
            if cfg.split == 'train':
                ap_transform = get_ap_transforms(cfg.at_cfg) if hasattr(cfg, 'at_cfg') else None
                ...
                valid_set_1 = Sintel(cfg.root_sintel, n_frames=cfg.n_frames, type='clean',
                                     subsplit=cfg.subsplit,
                                     transform=valid_input_transform,
                                     target_transform={'flow': sep_transforms.ArrayToTensor()}
                                     )
                valid_set_2 = Sintel(cfg.root_sintel, n_frames=cfg.n_frames, type='final',
                                     subsplit=cfg.subsplit,
                                     transform=valid_input_transform,
                                     target_transform={'flow': sep_transforms.ArrayToTensor()}
                                     )
